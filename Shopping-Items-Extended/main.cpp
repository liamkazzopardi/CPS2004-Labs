...
void saveToFile(const vector<unique_ptr<shoppingItems>> &items, const string &filename) {
...
vector<unique_ptr<shoppingItems>> loadFromFile(const string &filename) {
    ifstream in(filename);
    vector<unique_ptr<shoppingItems>> items;
    while (in) {
        unique_ptr<shoppingItems> item(shoppingItems::restore(in));
        if (item) items.push_back(move(item));
    }
    return items;
}
    ...
    do {
        ...
        if (option == 1) {
            ...
            if (itemType == 1) {
                ...
                cin >> expirationDate;
                items.push_back(make_unique<Grocery>(name, price, quantity, expirationDate));
            } 
            else if (itemType == 2) {
                string warrantyPeriod;
                cout << "Enter warranty period (years): ";
                cin >> warrantyPeriod;
                items.push_back(make_unique<Electronics>(name, price, quantity, warrantyPeriod));
            } 
            else if (itemType == 3) {
                string ISBN;
                cout << "Enter ISBN: ";
                cin >> ISBN;
                items.push_back(make_unique<Books>(name, price, quantity, ISBN));
            } 
            else if (itemType == 4) {
                string size, color;
                cout << "Enter size: ";
                ...
                cout << "Enter color: ";
                cin >> color;
                items.push_back(make_unique<Clothing>(name, price, quantity, size, color));
            } 
            else if (itemType == 5) {
                int suitableAge;
                cout << "Enter suitable age (in years): ";
                cin >> suitableAge;
                items.push_back(make_unique<ToyItems>(name, price, quantity, suitableAge));
            } 
            else {
